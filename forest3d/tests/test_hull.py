...
class TestHull(unittest.TestCase):
    ...
    def test_stem_x_critical_points_isolation(self):
        ...
        tree1_critical_points = np.vstack((tree1.crown_apex, tree1.crown_base,
                                           tree1.peripheral_points)).T

        tree2_critical_points = np.vstack((tree2.crown_apex, tree2.crown_base,
                                           tree2.peripheral_points)).T

        # make sure x coordinates are different
        self.assertFalse(
            np.allclose(tree1_critical_points[0], tree2_critical_points[0]))
        # make sure y and z coordinates are the same
        self.assertTrue(
            np.allclose(tree1_critical_points[1:3],
    ...
    def test_stem_y_critical_points_isolation(self):
        ...
        self.assertTrue(
            np.allclose(tree1_critical_points[[0, 2]],
                        tree2_critical_points[[0, 2]]))
        # make sure y coordinates are different
        self.assertFalse(
            np.allclose(tree1_critical_points[1], tree2_critical_points[1]))
    ...
    def test_stem_z_critical_points_isolation(self):
        ...
        self.assertTrue(
            np.allclose(tree1_critical_points[0:1],
                        tree2_critical_points[0:1]))
        # make sure z coordinates are different
        self.assertFalse(
            np.allclose(tree1_critical_points[2], tree2_critical_points[2]))
